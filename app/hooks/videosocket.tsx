...
export function SocketProvider({ children }: any) {
  ...
  const [peerConnection, setPeerConection] = useState<RTCPeerConnection | null>(
    null
  );
      ...
      ws.onmessage = async (event) => {
        ...
        if (message.type === "incoming_call") {
          ...
          const askuser = confirm(
            `Incoming call from ${message.fromUserID}. Accept?`
          );
  ...
  return (
    <SocketContext.Provider value={{ createCall }}>
      {children}
